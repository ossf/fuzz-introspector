...
def detect_branch_level_blockers(fuzz_profile: fuzz_data_loader.FuzzerProfile) -> List[Any]:
    ...
    for branch_string in coverage.branch_cov_map:
        ...
        try:
            ...
            logger.error(f"branch-profiling: error getting function name")
            continue

        if not function_name in functions_profile:
            logger.error(f"branch-profiling: func name not in functions_profile")
            continue
