...
def analysis_get_optimal_targets(merged_profile):
    ...
    optimal_set = set()
    target_fds = list()
    for fd in reversed(sorted(merged_profile['all_function_data'], key=lambda x: len(x['functionsReached']))):
...
def analysis_synthesize_simple_targets(merged_profile):
    ...
    var_idx = 0
    func_count = len(merged_profile['all_function_data'])
    if func_count > 20000:
        max_count = 1
    ...
    while curr_count < max_count:
        ...
        print(". Done")
        for tmp_ff in new_merged_profile['all_function_data']:
