...
pub fn generate_yaml(functions: &[FunctionInfo], fuzzing_harnesses: &[String]) -> io::Result<()> {
    // Generate yaml per harness
    for harness in fuzzing_harnesses {
        ...
        let report = FuzzerReport {
            ...
            all_functions: FunctionSection {
                ...
                elements: functions.to_vec(),
            },
        };

        // Convert and save to yaml file
        let yaml_data = serde_yaml::to_string(&report).expect("");
