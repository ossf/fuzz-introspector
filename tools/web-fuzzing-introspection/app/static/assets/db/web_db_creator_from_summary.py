...
def extend_db_timestamps(db_timestamp):
    existing_timestamps = []
    if os.path.isfile('db-timestamps.json'):
        with open('db-timestamps.json', 'r') as f:
            try:
...
def extend_project_timestamps(project_timestamps):
    existing_timestamps = []
    if os.path.isfile('all-project-timestamps.json'):
...
def analyse_set_of_dates(dates):
    for date in dates:
        function_list, project_timestamps, db_timestamp = analyse_list_of_projects(date, project_list)
